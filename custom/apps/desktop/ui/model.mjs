...
export function formatBalanceAmount(value, currency) {
...
export function buildBalanceCards({ providerUsage, providerSetup } = {}) {
  const configured = new Set(
    (providerSetup?.providers || [])
      .filter((provider) => provider.configured)
      .map((provider) => provider.id),
  ...
  for (const provider of providerUsage?.providers || []) {
    if (!configured.has(provider.id)) continue;
    for (const metric of provider.account?.metrics || []) {
      if (metric?.kind !== "balance") continue;
      const amount = formatBalanceAmount(metric.value, metric.currency);
      if (!amount) continue;
      cards.push({
        key: `${provider.id}:balance`,
        providerId: provider.id,
        providerName: provider.displayName || provider.id,
        label: metric.label || "API balance",
        amount,
...
function accountWindowMetric(windowKey, metric) {
  const window = quotaWindow(metric);
  return {
    key: windowKey,
    label: window?.label || (windowKey === "primary" ? "5-hour limit" : "Weekly limit"),
...
export function buildLimitGroups({ account, providerUsage, providerSetup } = {}) {
  const groups = [];
  const addAccount = (key, name, usage) => {
    const metrics = [];
    if (usage?.primary) metrics.push(accountWindowMetric("primary", usage.primary));
    if (usage?.secondary) metrics.push(accountWindowMetric("secondary", usage.secondary));
    const monthlyLimit = Number(usage?.monthly?.limit);
    const monthlyUsed = Number(usage?.monthly?.used);
    if (Number.isFinite(monthlyLimit) && monthlyLimit > 0 && Number.isFinite(monthlyUsed)) {
      const unit =
        typeof usage.monthly.unit === "string" && usage.monthly.unit ? ` ${usage.monthly.unit}` : "";
      metrics.push({
        key: "monthly",
        label: "Monthly limit",
        percent: clampPercent((monthlyUsed / monthlyLimit) * 100),
        amount: null,
        detail: `${Math.round(monthlyUsed).toLocaleString("en-US")} / ${monthlyLimit.toLocaleString("en-US")}${unit}`,
        resetAt: null,
      });
    }
    if (metrics.length) groups.push({ key, name, metrics });
  };
  addAccount("openai-native", "ChatGPT (native)", account);
  addAccount("chatgpt-login", "ChatGPT (personal)", account?.personal);
  ...
  for (const provider of providerUsage?.providers || []) {
    ...
    if (metrics.length) groups.push({ key: provider.id, name: provider.displayName || provider.id, metrics });
  }
  return groups;
}

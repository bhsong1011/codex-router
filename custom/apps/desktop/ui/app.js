...
function startPanel() {
  ...
  function renderQuotas() {
    const groups = buildLimitGroups(state);
    elements.quotaCards.innerHTML = groups.length
      ? groups
          .map(
            (group) => `<article class="quota-card limit-card">
              <header><span class="limit-name">${escapeHtml(group.name)}</span></header>
              ${group.metrics
                .map((metric) => {
                  const left =
                    metric.percent === null
                      ? null
                      : Math.min(100, Math.max(0, 100 - metric.percent));
                  const value =
                    metric.amount ?? (left === null ? "—" : `${Math.round(left)}% left`);
                  const bar =
                    left === null
                      ? ""
                      : `<progress max="100" value="${left}" aria-label="${escapeHtml(metric.label)} ${Math.round(left)} percent left"></progress>`;
                  const note = metric.resetAt ? formatReset(metric.resetAt) : metric.detail || "";
                  const reset = note
                    ? `<span class="limit-reset">${escapeHtml(note)}</span>`
                    : "";
                  return `<div class="limit-row">
                    <span class="limit-label">${escapeHtml(metric.label)}</span>
                    <span class="limit-value">${escapeHtml(value)}</span>
                  </div>${bar}${reset}`;
                })
                .join("")}
            </article>`,

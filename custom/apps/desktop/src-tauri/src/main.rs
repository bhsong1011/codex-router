...
async fn account_usage(state: State<'_, RouterState>) -> Result<Value, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let mut native = run_control_json(&router, &["account", "--json"], None)?;
        if let Some(home) = native_codex_home() {
            if let Some(monthly) = read_monthly_usage(&home) {
                if let Value::Object(map) = &mut native {
                    map.insert("monthly".into(), monthly);
                }
            }
        }
        if let Some(home) = personal_codex_home() {
            if let Ok(mut personal) =
                run_control_json_with_home(&router, &["account", "--json"], None, &home)
            {
                if let Some(monthly) = read_monthly_usage(&home) {
                    if let Value::Object(map) = &mut personal {
                        map.insert("monthly".into(), monthly);
                    }
                }
                if let Value::Object(map) = &mut native {
                    map.insert("personal".into(), personal);
                }
            }
        }
        Ok::<Value, String>(native)
    })
    .await
...
fn run_control_json_with_home(
    state: &RouterState,
    args: &[&str],
    stdin: Option<&[u8]>,
    codex_home: &Path,
) -> Result<Value, String> {
    let output = run_control_with_home(state, args, stdin, Some(codex_home))?;
    serde_json::from_slice(&output)
        .map_err(|_| "Model Router returned an unreadable response.".to_string())
}

fn personal_codex_home() -> Option<PathBuf> {
    if let Some(configured) = env::var_os("CODEX_ROUTER_CHATGPT_LOGIN_HOME") {
        if !configured.is_empty() {
            let home = PathBuf::from(configured);
            return home.join("auth.json").is_file().then_some(home);
        }
    }
    let home = PathBuf::from(env::var_os("HOME")?).join(".codex-personal");
    home.join("auth.json").is_file().then_some(home)
}

fn native_codex_home() -> Option<PathBuf> {
    let home = match env::var_os("CODEX_HOME") {
        Some(configured) if !configured.is_empty() => PathBuf::from(configured),
        _ => PathBuf::from(env::var_os("HOME")?).join(".codex"),
    };
    home.join("auth.json").is_file().then_some(home)
}

fn read_monthly_usage(codex_home: &Path) -> Option<Value> {
    let node = resolve_node()?;
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../monthly-usage.mjs");
...
fn run_control(
    ...
    run_control_with_home(state, args, stdin, None)
}

fn run_control_with_home(
    state: &RouterState,
    args: &[&str],
    stdin: Option<&[u8]>,
...
fn resolve_node() -> Option<OsString> {
    ...
    for directory in env::split_paths(&augmented_path()) {
        ...
        if candidate.is_file() && node_is_supported(&candidate) {
...
fn node_is_supported(path: &Path) -> bool {
    Command::new(path)
        ...
        .and_then(|raw| parse_node_version(&raw))
        .map(node_version_supported)
        .unwrap_or(false)
}

fn node_version_supported((major, minor): (u32, u32)) -> bool {
...
fn augmented_path() -> OsString {
    ...
    augmented_path_from(current.as_deref())
}

fn augmented_path_from(current: Option<&OsStr>) -> OsString {
...
mod tests {
    ...
    fn parses_node_versions() {
        ...
        assert_eq!(parse_node_version("12.22.9"), Some((12, 22)));
        assert_eq!(parse_node_version("22"), Some((22, 0)));
        assert_eq!(parse_node_version("not-a-version"), None);
    }

    #[test]
    fn rejects_node_older_than_22_19() {
        assert!(!node_version_supported((12, 22)));
        assert!(!node_version_supported((22, 18)));
        assert!(node_version_supported((22, 19)));
        assert!(node_version_supported((23, 0)));
    }

    #[test]
    fn keeps_inherited_path_ahead_of_fallbacks() {
        let path = augmented_path_from(Some(OsStr::new("/users/me/.nvm/bin")));
        let entries: Vec<_> = env::split_paths(&path).collect();
        assert_eq!(entries.first(), Some(&PathBuf::from("/users/me/.nvm/bin")));
    }
}
